...
1. 默认，可以没有LSM模块注册，那么security_hook_heads里的所有链表头都是空的
2. 哪个LSM钩子生效就取决于是否有在对应钩子的链表头添加函数指针节点
3. 每个LSM模块一般不会把所有钩子都注册，常见的selinux、apparmor都没有券注册
4. 定义，请注意，这里说的是定义

```c
  7 /*
  6  * Initializing a security_hook_list structure takes
  5  * up a lot of space in a source file. This macro takes
  4  * care of the common case and reduces the amount of
  3  * text involved.
  2  */
  1 #define LSM_HOOK_INIT(HEAD, HOOK) \
...
在 Linux 6.6 内核中，call_void_hook 和 call_int_hook（你提到的 call_init_hook 应该是笔误，实际上是 call_int_hook，用于返回 int 的钩子）是 LSM 框架的核心宏。
它们位于 include/linux/lsm_hooks.h 中。

核心机制：静态调用链 (Static Call Chain)

从 Linux 5.7 开始，LSM 框架不再使用传统的“遍历链表调用函数指针”的方式，而是改用 static_call 机制。
1. 旧方式：每次调用都要遍历 security_hook_list 链表，性能较差。
2. 新方式：编译器在编译期生成优化的调用路径。如果只有一个 LSM 模块注册了该钩子，直接跳转；如果有多个，生成一个高效的调度器。这极大地减少了安全钩子带来的性能开销。
